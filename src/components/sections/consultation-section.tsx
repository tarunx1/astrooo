          ...
          <GlassCard variant="glass" spotlight={true} className="p-6" key={astrologer.name}>
            ...
            <div className="mt-6 grid grid-cols-3 gap-2">
              {[Phone, MessageCircle, Video].map((Icon, index) => (
                <button className="grid min-h-11 place-items-center rounded-md border border-white/10 bg-background/60 text-premium transition hover:border-premium hover:bg-background" key={index} type="button">
                  <Icon size={18} />
                </button>
              ))}
            </div>
