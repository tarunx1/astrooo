      ...
      <div className="grid gap-5 lg:grid-cols-[0.9fr_1fr_0.9fr]">
          ...
          <form className="mt-7 grid gap-4">
            {["Full name", "Birth date", "Birth time", "Birth place"].map((label) => (
              <label className="grid gap-2 caption text-foreground-secondary" key={label}>
                {label}
                <input
                  className="min-h-11 rounded-md border border-border-strong bg-background px-3 body-sm text-foreground outline-none transition focus:border-accent-cyan"
        ...
        <div className="border border-border bg-surface p-6">
          <div className="flex items-end justify-between gap-4">
            ...
            <a className="text-sm font-semibold text-premium" href="/calculators">
              View all
            </a>
          </div>
          <div className="mt-6 divide-y divide-border">
            {calculators.map((calculator) => {
              ...
              return (
                <a className="flex items-center gap-4 py-4" href="/calculators" key={calculator.title}>
                  <span className="grid size-12 place-items-center rounded-md border border-border bg-background text-premium">
                    <Icon size={20} />
                  </span>
                  <span>
